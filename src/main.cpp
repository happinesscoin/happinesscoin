...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Happinesscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Happinesscoin Signed Message:\n";
...
{
    // Happinesscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // nSubsidy >>= (nHeight / 840000); // Happinesscoin: 840k blocks in ~4 years
...
//    nSubsidy>>=count;
    ...
    else if(nHeight>1&&nHeight<30*3*1440)
        nSubsidy=2.08*COIN;
    else if(nHeight>=30*3*1440)
        nSubsidy=0.069*COIN;
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 30 *60; // Happinesscoin: 30min
static const int64 nTargetSpacing = 1 * 60; // Happinesscoin:30s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Happinesscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Happinesscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Happinesscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Created By zaobi.org .QQ:260682605 ,欢迎咨询!2016-4-11";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x45fa65b0695f9f02eef70a3b39449a11c7a2921e3129edf4593f40d2b1aad205"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Happinesscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("happinesscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("happinesscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static happinesscoinMiner(CWallet *pwallet)
{
    printf("happinesscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("happinesscoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running happinesscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("happinesscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&happinesscoinMiner, pwallet));
}
